...
for(filename in c(
  ...
  minintensity = min(xx["aveIntensity"]);
  maxintensity = max(xx["aveIntensity"]);
  means<-c();
  binindex<-c();
  binstart<-minintensity;
  while(binstart < maxintensity){
    rs <- data.frame();
    binend<-binstart;
    while(nrow(rs) < binsize){
      binend<-binend+binwidth;
      rs <- subset(xx, (xx["aveIntensity"] >= binstart));
      rs <- subset(rs, (rs["aveIntensity"] <= binend));
      if(binend > maxintensity){
        break;
      }
    }
