...
DisplayComparison<-function(screenIndex, g, indexs, mybreaks, legends, colors, title){
  ...
  g1<-log(g[,indexs[1]]/g[,indexs[2]]);
  g2<-log(g[,indexs[3]]/g[,indexs[4]]);
  mean(g1);
  ...
  plot(0,xlim=c(-3,3),ylim=c(0,maxCount),type="n",xlab="log(Ratio)",ylab="Count",main=title);
  ...
  legend(1,maxCount,legend=legends,lty=4,col=colors);
}

ITraqDistribution<-function(inputfilename, title){
