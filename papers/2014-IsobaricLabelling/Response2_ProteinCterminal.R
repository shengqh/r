...
data<-read.table("Response_2_ProteinCTerminal.tsv", header=T,row.names=1,sep="\t")

table(is.na(data$AfterScore))["TRUE"]
table(is.na(data$BeforeScore))["TRUE"]

mean(common$BeforeScore)
mean(common$AfterScore)

common<-data[!is.na(data$BeforeScore) & !is.na(data$AfterScore),]

wilcox.test(common$BeforeScore, common$AfterScore, paired=T)
nrow(data)
