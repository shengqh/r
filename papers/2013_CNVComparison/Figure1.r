...
names<-c("aCGH","exomeCopy", "ExomeDepth","cn.MOPS","CoNIFER")
filenames<-c("cgh.txt","exomeCopy.txt","ExomeDepth.txt","cnmops.txt","conifer.txt")
ncount<-length(names)

datatype<-lapply(c(1:ncount), FUN=function(x){
  return (readtypedata(filenames[x],names[x]))
})
typedata<-rbind(datatype[[1]], 
                datatype[[2]], 
                datatype[[3]], 
                datatype[[4]],
                datatype[[5]]
...
datalen<-lapply(c(1:ncount), FUN=function(x){
  return (readlendata(filenames[x],names[x]))
})
lendata<-rbind(datalen[[2]][,c(1,6)], 
               datalen[[3]][,c(1,6)], 
               datalen[[1]][,c(1,6)],
               datalen[[5]][,c(1,6)],
               datalen[[4]][,c(1,6)]
)
lendata[,2]<-log10(lendata[,2])
names(lendata)[2]<-"Length"

pdf(file="figure1.pdf", width=12, height=6)
#png(file="figure1.png",width=4000, height=2000, res=300)
...
g2<-qplot(Method, Length, data=lendata, geom=c("boxplot"), fill=Method, ylab="log10(CNV length)", xlab="") + theme_bw() + 
