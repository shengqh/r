...
outfilename=paste(dir, "charge.jpg", sep="");
...
for(i in 1:4){
  ...
  for(j in 2:3){
    ...
    title<-paste(filelabels[i]," Charge",j,"+",sep="");
    
    t1<-"T-M";
    g1<-subset(subset(subset(g,Decoy=="False"), Modified=="True"), Charge==j);
    t2<-"D-M";
    g2<-subset(subset(subset(g,Decoy=="True"), Modified=="True"), Charge==j);
    t3<-"T-U";
    g3<-subset(subset(subset(g,Decoy=="False"), Modified=="False"), Charge==j);
    t4<-"D-U";
    g4<-subset(subset(subset(g,Decoy=="True"), Modified=="False"), Charge==j);
