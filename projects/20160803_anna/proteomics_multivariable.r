...
varselect<-function(datamat, outcome, alpha, filename){
  set.seed(2016)
  ...
  write.csv(winners, filename)
}


# run lasso
totalValueCol<-ncol(d4) - 1

varselect(data.matrix(d4[,11:totalValueCol]), d4$PVH_PAH, alpha=1, "proteomics_multivariable_PVH_vs_PAH_winners.csv")
varselect(data.matrix(d5[,11:totalValueCol]), d5$PAH_NoPH, alpha=1, "proteomics_multivariable_PAH_vs_NoPH_winners.csv")
varselect(data.matrix(d6[,11:totalValueCol]), d6$PVH_Combined, alpha=1, "proteomics_multivariable_PVH_vs_Combined_winners.csv")
varselect(data.matrix(d7[,11:totalValueCol]), d7$PAH_Combined, alpha=1, "proteomics_multivariable_PAH_vs_Combined_winners.csv")
