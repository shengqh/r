...
setwd("H:/shengquanhu/projects/miRNA_v2")
...
upperQuantile<-function(mirnaCount){
  ups<-apply(mirnaCount, 2, function(x) quantile(x)[4])
  mups<-median(ups)
  mm<-lapply(c(1:ncol(mirnaCount)), function(x) {
    ll<-mirnaCount[,x] / ups[x] * mups
    c(unlist(ll))
  })
  mc<-data.frame(mm)
  colnames(mc)<-colnames(mirnaCount)
  rownames(mc)<-rownames(mirnaCount)
  return (mc)
}
...
mirnaTargetNormFile = "TCGA_mirnaseq_20130521_brca_count_norm_target.csv"

rnaseqNormFile = "TCGA_rnaseq_20130521_brca_count_norm.csv"