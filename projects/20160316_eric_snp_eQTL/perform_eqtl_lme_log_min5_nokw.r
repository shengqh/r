  ...
  if(nrow(filtered) > 0){
    ...
    apply(filtered, 1, function(x){
      ...
      g<-ggplot(dat, aes(GenoType, GeneExpression)) + 
        ...
        ggtitle(paste0("association adjust pvalue =",format.pval(associationFDR)))
      print(g)
      dev.off()
    })
  }
...
perform_eqtls<-function(eqtls_file_name_noext, snp_info, snp_genotypes, gene_covariants, fdr_threshold=0.05){
  ...
  list(eqtls=eqtls, sig_eqtls=sig_eqtls)
}
...
for(index in c(1:2)){
  ...
  for(genesymbol in genesymbols){
    ...
    cis_eqtls=cis_result$sig_eqtls
    ...
    trans_eqtls=trans_result$sig_eqtls
